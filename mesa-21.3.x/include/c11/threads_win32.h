...
# include <stdlib.h>
static inline int timespec_get(struct timespec *, int);
#else
#include <errno.h>
...
#ifdef IPRT_NO_CRT
static DECLCALLBACK(int) impl_thrd_routine(RTTHREAD hThreadSelf, void *p)
#else  /* VBox */
...
thrd_create(thrd_t *thr, thrd_start_t func, void *arg)
{
    ...
    pack->arg = arg;
#ifdef IPRT_NO_CRT
    rc = RTThreadCreateF(&hThread, impl_thrd_routine, pack, 0, RTTHREADTYPE_DEFAULT, RTTHREADFLAGS_WAITABLE,
                         "MESA-%u", s_iThreadNo++);
    if (RT_SUCCESS(rc))
        *thr = (thrd_t)hThread;
...
// 7.25.5.3
#ifndef IPRT_NO_CRT /* Detaching is not implemented, could easily be done, though. */
static inline int
...
#ifdef IPRT_NO_CRT
    return thr0 == thr1
        || RTThreadGetNativeHandle(thr0) == RTThreadGetNativeHandle(thr1);
#else
...
thrd_join(thrd_t thr, int *res)
{
    ...
    CloseHandle(thr);
#else
    int rc = RTThreadWait(thr, RT_INDEFINITE_WAIT, res);
    if (RT_FAILURE(rc))
        return thrd_error;
#endif
    return thrd_success;
...
#ifdef IPRT_NO_CRT
    {
        time_t const cMillies = impl_timespec2msec(time_point);
        RTThreadSleep((time_t)(RTMSINTERVAL)cMillies == cMillies  ? cMillies : RT_INDEFINITE_WAIT);
    }
#else
...
// 7.25.6.1
#if !defined(HAVE_TIMESPEC_GET) || defined(IPRT_NO_CRT)
static inline int
...
# ifdef IPRT_NO_CRT
        RTTIMESPEC Now;
        RTTimeSpecGetTimespec(RTTimeNow(&Now), ts);
# else
        ts->tv_sec = time(NULL);
        ts->tv_nsec = 0;
# endif
